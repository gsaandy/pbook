...
import { Plus, Upload, Search, Edit2, Trash2, Power, PowerOff } from 'lucide-react'
import type { SetupAndConfigurationProps } from './types'
...
export function SetupAndConfiguration({
  ...
  const [showInactiveEmployees, setShowInactiveEmployees] = useState(includeInactive)
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          {activeTab === 'employees' && (
            <label className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
              <input
                type="checkbox"
                checked={showInactiveEmployees}
                onChange={(e) => {
                  setShowInactiveEmployees(e.target.checked);
                  onInactiveFilterChange?.(e.target.checked);
                }}
                            ...
                            <div className="flex justify-end gap-2">
                              {onEditEmployee && (
                                <button
                                  ...
                                  className="p-1.5 text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                  aria-label="Edit employee"
                                  title={employee.role === 'super_admin' ? 'Super admin users cannot be edited' : 'Edit employee'}
                                >
                                  <Edit2 className="w-4 h-4" />
                                </button>
                              )}
                              {onToggleEmployeeStatus && (
                                <button
                                  ...
                                  disabled={employee.role === 'super_admin'}
                                  className="p-1.5 text-slate-600 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                  aria-label="Toggle employee status"
                                  title={employee.role === 'super_admin' ? 'Super admin users cannot be deactivated' : 'Toggle employee status'}
                                >
                                  {employee.status === 'active' ? (
                      ...
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex gap-1 ml-2">
                          {onEditEmployee && (
                            <button
                              onClick={() => onEditEmployee(employee.id)}
                              disabled={employee.email === 'gsaandy.dev@gmail.com'}
                              className="p-2 text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded disabled:opacity-50 disabled:cursor-not-allowed"
                              title={employee.email === 'gsaandy.dev@gmail.com' ? 'This user is protected and cannot be edited' : 'Edit employee'}
                            >
                              <Edit2 className="w-4 h-4" />
                            </button>
                          )}
                          {onToggleEmployeeStatus && (
                            <button
                              ...
                              disabled={employee.email === 'gsaandy.dev@gmail.com'}
                              className="p-2 text-slate-600 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded disabled:opacity-50 disabled:cursor-not-allowed"
                              title={employee.email === 'gsaandy.dev@gmail.com' ? 'This user is protected and cannot be deactivated' : 'Toggle employee status'}
                            >
                              {employee.status === 'active' ? (
                                <PowerOff className="w-4 h-4" />
                              ) : (
