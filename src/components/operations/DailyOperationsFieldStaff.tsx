...
import { Search, Banknote, Smartphone, FileText, ChevronRight, Wallet } from 'lucide-react'
import type { DailyOperationsFieldStaffProps, CollectionFormData } from '../../../product-plan/sections/daily-operations/types'
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="max-w-2xl mx-auto pb-6">
        ...
        {/* Search Results Modal - Note: This requires shops to be passed as prop */}
        {showSearchModal && searchQuery && (
          <div className="fixed inset-0 bg-black/50 z-40 flex items-start justify-center p-4 pt-20">
            <div className="bg-white dark:bg-slate-800 rounded-xl w-full max-w-md max-h-[70vh] overflow-y-auto shadow-xl">
              <div className="sticky top-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400"
                    autoFocus
                  />
                </div>
              </div>
              <div className="p-4">
                <p className="text-sm text-slate-600 dark:text-slate-400 text-center py-4">
                  Shop search results will appear here when shops are available.
                  <br />
                  Click on a shop to log collection.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
          {assignedRoute.shops.length === 0 ? (
            <div className="p-8 text-center text-slate-500 dark:text-slate-400">
              No shops in this route
            </div>
          ) : (
            assignedRoute.shops.map((shop) => (
              <button
                key={shop.id}
