...
export const Route = createFileRoute("/setup/routes/$id/edit")({
...
function EditRoutePage() {
  ...
  const route = useQuery(api.queries.getRoute, { id: id as any });
  const shops = useQuery(api.queries.getShops) ?? [];
  const updateRoute = useMutation(api.mutations.updateRoute);
    ...
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Update route details and shop assignments
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
        ...
        <div>
          ...
          <div className="border border-slate-300 dark:border-slate-600 rounded-lg p-4 max-h-64 overflow-y-auto">
            {shops.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">No shops available.</p>
            ) : (
